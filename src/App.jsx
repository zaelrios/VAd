...
export default function App() {
  ...
  // --- PASO 1: VERIFICAR / INICIAR SESIÓN ---
  ...
  // --- PASO 2: REGISTRO OBLIGATORIO NOMBRE Y APELLIDO ---
  const handleCompleteRegistration = async (e) => {
    e.preventDefault();
    
    const nameParts = registrationName.trim().split(/\s+/);
    if (nameParts.length < 2) {
      setAuthError('Es obligatorio ingresar nombre y apellido reales.');
      return;
    }
    ...
    try {
      ...
      console.error(error);
      setAuthError('No se pudo crear la cuenta. Revisa tu conexión.');
    } finally {
      setAuthLoading(false);
    }
  };
  ...
  const handleSearchSubmit = async (e) => {
    ...
    if (startTime >= endTime) { setSearchError('La hora límite debe ser mayor a la de inicio.'); return; }
    ...
    try {
      const { data, error } = await supabase
        ...
        .insert([{
          ...
          nombre: currentUser.nombre, // Se envía el nombre a la nueva columna
          fecha: searchDate,
      ...
      console.error(error);
      setSearchError('No se pudo publicar la búsqueda.');
    }
  };
  ...
  const handleBookSubmit = (e) => {
    ...
    setBookError('');
    if (!isLoggedIn) { setTab('auth'); return; }
    if (bookStart >= bookEnd) { setBookError('Revisa el horario de reserva.'); return; }
  ...
  return (
    <div className="min-h-screen bg-[#F8F7F2] text-[#1A1C1E] font-sans pb-32 selection:bg-[#29C454]/30">
      <main className="pt-10 px-6 max-w-lg mx-auto w-full flex flex-col items-center">
        ...
        {tab === 'home' && (
          <div className="w-full space-y-10 animate-in fade-in slide-in-from-bottom-6 duration-700 text-center">
            <section className="flex flex-col items-center">
              <h2 className="text-[#1A1C1E] font-black uppercase tracking-[0.4em] text-[15px] mb-4">Donde el tennis se vive</h2>
            ...
            {!isRegistering ? (
                ...
                <form onSubmit={handleAuthSubmit} className="space-y-6">
                  <div className="space-y-2 text-left">
                    ...
                    <div className="flex gap-2">
                      <select value={phonePrefix} onChange={(e) => setPhonePrefix(e.target.value)} className="w-1/3 bg-[#FFFFFF] border border-[#1A1C1E]/10 rounded-2xl px-2 py-4 text-[#1A1C1E] font-bold shadow-sm">
                  ...
                  <button type="submit" disabled={authLoading} className="w-full bg-[#29C454] text-white py-5 rounded-2xl font-black italic uppercase text-sm shadow-lg shadow-[#29C454]/30 active:scale-95 transition-all">
              ...
              <div className="space-y-8 animate-in fade-in">
                <div className="text-center">
                  ...
                  <p className="text-[#1A1C1E]/60 text-sm">Crea tu perfil oficial para empezar a jugar.</p>
                </div>
                <form onSubmit={handleCompleteRegistration} className="space-y-6">
                  <div className="space-y-2 text-left">
                    <label className="text-[10px] font-black text-[#1A1C1E]/50 uppercase tracking-widest ml-2">Nombre y Apellido</label>
                    <input type="text" required placeholder="Ej: Zael Rios" value={registrationName} onChange={(e) => setRegistrationName(e.target.value)} className="w-full bg-[#FFFFFF] border border-[#1A1C1E]/10 rounded-2xl px-5 py-4 text-[#1A1C1E] font-bold tracking-wider focus:outline-none focus:border-[#29C454]" />
                  </div>
                  {authError && <div className="bg-red-50 text-red-600 border border-red-200 p-3 rounded-xl text-xs font-bold text-center">{authError}</div>}
                  <button type="submit" disabled={authLoading} className="w-full bg-[#1A1C1E] text-white py-5 rounded-2xl font-black italic uppercase text-sm shadow-lg">
                    {authLoading ? 'Creando...' : 'Completar Registro ➜'}
                  </button>
                </form>
              </div>
            )}
        ...
        {/* VISTA: BUSCAR RIVAL */}
        {tab === 'buscar' && (
          <div className="w-full space-y-6 animate-in slide-in-from-bottom-8 mt-4">
            <form onSubmit={handleSearchSubmit}>
              <div className="bg-[#FFFFFF] border border-[#1A1C1E]/10 rounded-[2.5rem] p-6 shadow-sm space-y-6">
                ...
                <div className="space-y-3 text-left w-full">
                  <label className="text-[10px] font-black text-[#1A1C1E]/50 uppercase tracking-widest ml-2">Horario de disponibilidad</label>
              ...
              <div className="pt-6 flex flex-col items-center">
                <button type="submit" className="w-fit px-8 bg-[#29C454] text-white py-5 rounded-2xl font-black italic uppercase text-sm shadow-lg shadow-[#29C454]/30 active:scale-95 transition-all">
                  Buscar rival ➜
                </button>
              </div>
            </form>
            ...
            {isLoggedIn && activeSearches.length > 0 && (
              <div className="pt-8 space-y-4 animate-in fade-in">
                ...
                <div className="space-y-3">
                  {activeSearches.map((search) => (
                    <div key={search.id} className="bg-[#FFFFFF] border border-[#29C454]/30 rounded-2xl p-4 flex items-center justify-between shadow-sm relative overflow-hidden">
                      ...
                      <button onClick={() => handleCancelSearch(search.id)} className="w-10 h-10 bg-[#F8F7F2] text-[#1A1C1E]/40 rounded-full flex items-center justify-center hover:bg-red-50 hover:text-red-500 transition-colors">✕</button>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {tab === 'reservar' && (
          <div className="w-full space-y-6 animate-in slide-in-from-bottom-8 mt-4">
            <div className="text-center">
              ...
              <p className="text-[#1A1C1E]/60 text-sm">Asegura tu lugar en el club.</p>
            </div>
            <form onSubmit={handleBookSubmit}>
              <div className="bg-[#FFFFFF] border border-[#1A1C1E]/10 rounded-[2.5rem] p-6 shadow-sm space-y-6">
                ...
                <div className="space-y-3 text-left w-full">
                  <label className="text-[10px] font-black text-[#1A1C1E]/50 uppercase tracking-widest ml-2">Horario</label>
                  <div className="grid grid-cols-2 gap-3 w-full">
                    <input type="time" value={bookStart} onChange={(e) => setBookStart(e.target.value)} required className="w-full bg-[#F8F7F2] border border-[#1A1C1E]/10 rounded-2xl py-4 text-[#1A1C1E] font-black text-center focus:outline-none focus:border-[#29C454]" />
                    <input type="time" value={bookEnd} onChange={(e) => setBookEnd(e.target.value)} required className="w-full bg-[#F8F7F2] border border-[#1A1C1E]/10 rounded-2xl py-4 text-[#1A1C1E] font-black text-center focus:outline-none focus:border-[#29C454]" />
                  </div>
                </div>
              </div>
              <div className="pt-6">
                <button type="submit" className="w-full bg-[#29C454] text-white py-5 rounded-2xl font-black italic uppercase text-sm shadow-lg shadow-[#29C454]/30 active:scale-95 transition-all">
                  Pagar Reserva ➜
                </button>
              </div>
            </form>
            ...
            <div className={`bg-[#FFFFFF] border border-[#1A1C1E]/10 rounded-[2.5rem] p-8 shadow-sm flex flex-col items-center text-center relative overflow-hidden ${!isLoggedIn && 'opacity-80'}`}>
              ...
              <div className="flex gap-4 w-full mt-8 border-t border-[#1A1C1E]/10 pt-6">
                <div className="flex-1">
                  ...
                  <p className="text-2xl font-black italic text-[#29C454]">{isLoggedIn && currentUser ? currentUser.elo : '1,000'}</p>
                </div>
                ...
                <div className="flex-1">
                  ...
                  <p className="text-2xl font-black italic text-[#1A1C1E]">{isLoggedIn && currentUser ? `${currentUser.confianza}%` : '100%'}</p>
                </div>
              </div>
            </div>
            {isLoggedIn ? (
              <button onClick={handleLogout} className="w-full bg-[#F8F7F2] border border-red-500/20 text-red-500 py-4 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-red-50 transition-colors">
                Cerrar Sesión
              </button>
            ) : (
